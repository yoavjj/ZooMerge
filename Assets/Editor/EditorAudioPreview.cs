...
[CustomPropertyDrawer(typeof(AudioSfxLibrarySO.MergeSfxEntry))]
public class MergeSfxEntryDrawer : PropertyDrawer
{
        ...
        GUIContent label)
    {
        ...
        if (cueProperty == null)
        {
            EditorGUI.HelpBox(
                position,
                "Could not find the merge cue field.",
                MessageType.Error
            );
        ...
        SfxMerge selectedCue =
            (SfxMerge)cueProperty.enumValueIndex;

        selectedCue = (SfxMerge)EditorGUI.EnumPopup(
            row,
            "Merge Cue",
            selectedCue
        );
